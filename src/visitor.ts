...
> {
    ...
    protected getEnumValue(enumName: string, enumOption: string): string {
        if (
            typeof this.config.enumValues === 'object' &&
            this.config.enumValues[enumName] &&
            typeof this.config.enumValues[enumName] === 'object' &&
            (this.config.enumValues[enumName] as any)[enumOption]
        ) {
            return (this.config.enumValues[enumName] as any)[enumOption] as string;
        }
    ...
    getDeprecationReason(directive: DirectiveNode): string {
        ...
        if (hasArguments && args[0].value.kind === Kind.STRING) {
    ...
    protected resolveInputFieldType(
        ...
        let result: CSharpFieldType | null = null;
